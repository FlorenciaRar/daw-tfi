...
export class RespuestasService {
  ...
  async crearRespuesta(id:number, dtoEncuesta: BuscarEncuestaDTO, dtoRespuesta: CrearRespuestaDTO): Promise<any> {
    const encuesta = await this.encuestasService.buscarEncuesta(id, dtoEncuesta.codigo, dtoEncuesta.tipo)
    ...
    const preguntasEnRespuesta = dtoRespuesta.respuestasAbiertas.map(ra => ra.idPregunta);
    const idsInvalidos = preguntasEnRespuesta.filter(id => !preguntasEnEncuesta.includes(id));

    if (idsInvalidos.length > 0) {
      throw new BadRequestException(`Las preguntas [${idsInvalidos.join(', ')}] no pertenecen a la encuesta.`);
    }
    ...
    const respuesta: Respuesta = this.respuestasRepository.create({respuestasOpciones, respuestasAbiertas, encuesta: encuesta, } )
